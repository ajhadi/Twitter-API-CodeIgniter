...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 */
	public function setting($identity, $old, $new)
	{
		$this->trigger_events('pre_change_password');
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		$group = $this->db->get_where($this->tables['users'], array('level' => $level))->row();
		if ($this->config->item('admin_group', 'ion_auth') === $group->level && $group_name !== $group->level)
		{
